...
import com.vault.fide.shared.R;
...
import com.vault.fide.shared.reflection.ReflectionUtils;
import com.vault.fide.shared.shell.command.runner.app.AppShell;
import com.vault.fide.shared.termux.file.TermuxFileUtils;
...
import com.vault.fide.shared.errors.Error;
import com.vault.fide.shared.android.PackageUtils;
import com.vault.fide.shared.termux.TermuxConstants.TERMUX_APP;
...
public class TermuxUtils {
     ...
     *
     * If your third-party app is targeting sdk `30` (android `11`), then it needs to add `com.vault.fide`
     * package to the `queries` element or request `QUERY_ALL_PACKAGES` permission in its
     * `AndroidManifest.xml`. Otherwise it will get `PackageSetting{...... com.vault.fide/......} BLOCKED`
     ...
     * <manifest
     *     <queries>
     *         <package android:name="com.vault.fide" />
     *    </queries>
     * </manifest>
     * }
     *
    ...
    /**
     * Check if Vault.Fide:API app is installed and enabled. This can be used by external apps that don't
     * share `sharedUserId` with the Vault.Fide:API app.
     *
     * @param context The context for operations.
    ...
    public static String geAPTInfoMarkdownString(@NonNull final Context context) {
        ...
        InputStream inputStream = context.getResources().openRawResource(com.vault.fide.shared.R.raw.apt_info_script);
        try {
